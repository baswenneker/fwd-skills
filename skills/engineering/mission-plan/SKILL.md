...
See [REFERENCE.md](REFERENCE.md) for the PRD + contract templates and slug rules, and [`fwd:mission-run`'s reference](../mission-run/REFERENCE.md) for the canonical `state.json` schema.
...
5. Bij materialisatie (het schrijven van `state.json` op de mission-branch): vul het top-level `rules_manifest` met `[{path, sha256}]` per regelbestand. Zie het schema in `../mission-run/REFERENCE.md` (schema v3) — documenteer het schema hier niet opnieuw.
...
```
echo '<user_testing-json>' | bash "${CLAUDE_SKILL_DIR}/../mission-run/scripts/boot-app.sh" --probe
```
...
- `state.json` — fill the skeleton: `gates[]`, `user_testing{}` (inclusief `plan_probe` wanneer de boot-preflight uit 4.7 slaagde — schema v5, additief), ordered `features[]` (each with `vc_ids`, `status: "pending"`), `milestones[]` (with `feature_ids`, `validation_status: "pending"`). Match the schema in `../mission-run/REFERENCE.md` exactly.

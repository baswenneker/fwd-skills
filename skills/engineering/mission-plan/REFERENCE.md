...
Templates and rules for the artifacts `fwd:mission-plan` produces. The canonical `state.json` schema lives in [`../mission-run/REFERENCE.md`](../mission-run/REFERENCE.md) — this file covers the two markdown artifacts and the slug.
...
- **Standing VCs (definitieplek: SKILL.md stap 4).** Every milestone carries three standing `scrutiny-review` assertions — comment hygiene (VC-4 shape: no mission-internal codes in committed comments/docstrings/commit messages, standalone-readable; norm: CONTEXT.md "Codecommentaar"), test quality (VC-5 shape: tests import the real production code, no tautological asserts, ≥1 test through the public entrypoint; dependency-mocks allowed), and design budget (VC-6 shape: the `mission.md` budget lists copied verbatim into the assertion text). Explicit user agreements land as **afspraken-VC's** — only agreements the user actually made, deliberately pinning implementation choices. The reviewer fails each hard; full definitions + sjablonen in SKILL.md step 4.

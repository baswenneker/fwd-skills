...
Eén stap per beurt, bewijs vóór uitleg, commit pas na akkoord. **Jij (hoofdsessie) schrijft de code zélf** — geen coder-subagent; uitleg uit eerste hand. Vers oordeel per stap: `fwd-skills:fwd-steps-reviewer` (read-only, verse context); tussenbalans elke 4 stappen: `fwd-skills:fwd-steps-doubt`. Werk in een eigen worktree — setup + werkafspraken: stap 0; hoofd-checkout blijft vrij voor parallel werk.
...
Getriggerd door `interim_review=due` uit `record-step.sh`. Spawn **beide doubt-agents parallel** (één bericht, twee Agent-calls, `subagent_type: fwd-skills:fwd-steps-doubt`). Elke prompt bevat alleen: worktree-pad `<WT>` als repo-root, de slug, de zojuist goedgekeurde stappen, en **één** vraag verbatim:

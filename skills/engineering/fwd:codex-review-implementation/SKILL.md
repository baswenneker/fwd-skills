---
name: fwd:codex-review-implementation
...
Codex als adversarial reviewer van een **diff**, na implementatie — getoetst tegen het plan waar die diff tegen aanleunt (indien een plan te vinden is). Zusterskill van [`fwd:codex-review-plan`](../fwd:codex-review-plan/SKILL.md); hergebruikt daarvan de resolver-script en de leesprocedure.
...
```
bash "${CLAUDE_SKILL_DIR}/../fwd:codex-review-plan/scripts/resolve-plan-artifact.sh" [<slug>]
```
...
**Plan-content lezen (indien een family is opgelost).** Exact de procedure uit [`fwd:codex-review-plan`'s Step 2](../fwd:codex-review-plan/SKILL.md) — dezelfde worktree-first → plain-file → `git show`-keuze op `path_or_branch`, dezelfde family→bestand-tabel (`mission.md`+`validation-contract.md`, of `plan.md`, of het contract zelf). Geen resultaat (exit `1`) → geen plan-content, Step 3 draait dan zonder plan-fidelity-vragen.

## Step 2 — Deterministische bestandsdekking (alleen `plan-contract`)

Uitsluitend wanneer Step 1 een `plan-contract` heeft opgelost — de enige family met een geverifieerd exact bestand-naar-plan-mechanisme. Hergebruik de twee rtk-gefilterde pipelines uit [`fwd:plan/REFERENCE.md`](../fwd:plan/REFERENCE.md) **verbatim**, tegen het contract's eigen Basis-commit:

...
- **Schema v5 is equally optional and additive.**
  ...
  - **`milestones[].advisories`** (`[{location, suggestion}]`): the reviewer's non-blocking simplicity findings for that milestone. Same replace-or-leave rule as `concerns` (schema v6, additive). They feed the walkthrough's advisories section and the eindrapport's "Open punten" table; they never affect `validation_status`, a verdict, or the circuit breaker.
...
Every milestone also carries three **standing `scrutiny-review` VCs** — comment hygiene (no mission-internal codes in committed comments/docstrings/commit messages, standalone-readable), test quality (real production imports, no vacuous asserts, ≥1 public-entrypoint test), design budget (the `mission.md` budget lists verbatim in the assertion) — plus **afspraken-VC's** for explicit user agreements (deliberately pinning implementation choices). `fwd:mission-plan` step 4 generates them per milestone and holds the definitions + sjablonen; a violation fails the milestone.
...
- **Subagents** (`fwd:mission-{coder,reviewer,user-tester}`): coder writes + commits; validators judge.
...
Agents are shipped at the plugin root in `agents/` and auto-discovered. They are referenced via `subagent_type` as `fwd:mission-coder`, `fwd:mission-reviewer`, `fwd:mission-user-tester`, `fwd:mission-scribe` — `fwd` is the plugin `name` in `.claude-plugin/plugin.json` and the agent files themselves carry no prefix (`agents/mission-coder.md`). If the plugin is ever installed under a different name, these identifiers change; this is the single place that fact is recorded.

`fwd:mission-scribe` (read-only, `model: haiku`) compiles the milestone walkthrough from facts the orchestrator already decided (`vc_results`, surviving concerns) — it runs the verification pass against the diff and returns text, but never judges: no handoff acceptance, no remediation, no verdicts of its own.

...
| engineering | [fwd:mission-plan](skills/engineering/fwd:mission-plan/SKILL.md) | Plan a multi-agent "mission" (a Claude Code take on Factory.ai Missions): scope a goal through conversation, write a PRD plus a validation contract (what "done" means, before any code), decompose into features and milestones, then create the mission branch and persist the plan on it. The interactive half; execution is handled by `/fwd:mission-run`. |
| engineering | [fwd:mission-run](skills/engineering/fwd:mission-run/SKILL.md) | Execute a planned mission — the resident orchestrator that drives features one at a time via fresh coder subagents, runs adversarial Scrutiny + User-Testing validators at milestone boundaries, and records a checkpoint after every unit so the mission resumes from any worktree or clone. Autonomous; wrap in `/loop` for long runs. Pass `status` for a read-only progress report. |
| engineering | [fwd:mission-run-parallel](skills/engineering/fwd:mission-run-parallel/SKILL.md) | Execute a planned mission in parallel waves — the opt-in, faster, riskier sibling of fwd:mission-run. Requires a v2 mission plan with depends_on fields. Spawns up to FWD_MISSION_MAX_PARALLEL (default 3) coder subagents per wave, each in its own slot worktree, then integrates sequentially onto the mission branch. Same state.json checkpoint format and validators as the serial runner; switching runners between ticks is safe. Typing the command is explicit risk acceptance. |
...
| engineering | [fwd:setup](skills/engineering/fwd:setup/SKILL.md) | Setup wizard for HeadingFWD's optional Claude Code conventions (smartlint Stop-hook, lessons memory file, gitignore entries, clear-context prompt on plan accept, disable commit/PR attribution). Asks scope + features in a single multiselect dialog, then runs every selected installer in batch and shows one consolidated summary. Copies bundled payloads and merges JSON snippets or markdown sections into the matching settings/CLAUDE.md. Idempotent. Explicit `/fwd:setup` invocation only. |
